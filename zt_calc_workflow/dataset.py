...
def zt_dataset_from_data(elec_prop_data, kappa_latt_data):
    ...
    for t in elec_t:
        if t not in kappa_latt_t:
            raise Exception(
                "Phono3py calculation must cover the temperature "
    ...
    for t in zt_data["t"]:
        ...
        for i, k in enumerate(["kappa_xx", "kappa_yy", "kappa_zz", "kappa_ave"]):
    ...
    key_sets = [
        ("kappa_el_xx", "kappa_latt_xx", "kappa_tot_xx"),
    ...
    for k_pf, k_kappa, k_zt in key_sets:
        zt_data[k_zt] = ((1.0e-3 * zt_data[k_pf]) / zt_data[k_kappa]) * zt_data["t"]
...
def zt_dataset_from_amset_phono3py_csvs(amset_file, phono3py_kappa_file):
    ...
    elec_prop_data = read_amset_csv(
        amset_file, convert_sigma_s_cm=True, calculate_pf_mw_m_k2=True
    )
