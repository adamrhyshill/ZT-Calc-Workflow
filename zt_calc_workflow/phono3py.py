...
_READ_PHONO3PY_KAPPA_KNOWN_HEADERS = list(_READ_PHONO3PY_KAPPA_HEADER_MAP.values())
...
def read_phono3py_kappa_csv(file_path):
    ...
    return read_validate_csv(
        file_path,
        header_map=_READ_PHONO3PY_KAPPA_HEADER_MAP,
        known_headers=_READ_PHONO3PY_KAPPA_KNOWN_HEADERS,
        known_headers_required=True,
    )


_READ_PHONO3PY_CRTA_HEADER_MAP = dict(
    _READ_PHONO3PY_KAPPA_HEADER_MAP,
    **{
        ...
        "(t^CRTA)_xx [ps]": "tau_crta_xx",
        "(t^CRTA)_yy [ps]": "tau_crta_yy",
        "(t^CRTA)_zz [ps]": "tau_crta_zz",
        "(t^CRTA)_yz [ps]": "tau_crta_yz",
        "(t^CRTA)_xz [ps]": "tau_crta_xz",
        "(t^CRTA)_xy [ps]": "tau_crta_xy",
        "(t^CRTA)_iso [ps]": "tau_crta_ave",
    }
)

_READ_PHONO3PY_CRTA_KNOWN_HEADERS = list(_READ_PHONO3PY_CRTA_HEADER_MAP.values())
...
def read_phono3py_crta_csv(file_path):
    ...
    return read_validate_csv(
        file_path,
        header_map=_READ_PHONO3PY_CRTA_HEADER_MAP,
