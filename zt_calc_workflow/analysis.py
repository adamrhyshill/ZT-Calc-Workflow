...
def match_data(calc_n, calc_t, calc_data_2d, to_match, mode="same_t", num_seeds=1):
    ...
    for n, t, val in to_match:
        ...
        if mode == "same":
            if n is None or t is None:
                raise Exception(
                    "mode = 'same' can only be used when both n "
            ...
            if t is None:
                raise Exception(
                    "mode = 'same_t' can only be used when T has " "been specified."
                )
            ...
            if num_seeds > 1:
                guesses = np.linspace(calc_log_n.min(), calc_log_n.max(), num_seeds)
            else:
            ...
            for log_n_0 in guesses:
                res = minimize(
                    _fit_func, [log_n_0], bounds=[(calc_log_n[0], calc_log_n[-1])]
                )
            ...
            if n is None:
                raise Exception(
                    "mode = 'same_n' can only be used when n has " "been specified."
                )
            ...
            for t in guesses:
                res = minimize(_fit_func, [t], bounds=[(calc_t.min(), calc_t.max())])
            ...
            match_res.append(
                (n, res_set[idx].x[0], interpolator((np.log10(n), res_set[idx].x[0])))
            )

        elif mode == "best_match":
            if num_seeds > 1:
                warnings.warn(
                    "Setting num_seeds > 1 with mode = 'best_match' "
            ...
            if num_seeds > 1:
                guess_n = np.linspace(calc_log_n.min(), calc_log_n.max(), num_seeds)

                guess_t = np.linspace(calc_t.min(), calc_t.max(), num_seeds)

                guesses = [(log_n, t) for log_n, t in product(guess_n, guess_t)]
            else:
            ...
            for log_n, t in guesses:
                res = minimize(
                    _fit_func,
                    [log_n, t],
                    bounds=[(calc_log_n[0], calc_log_n[-1]), (calc_t[0], calc_t[-1])],
                )

            match_res.append(
                (np.power(10.0, res.x[0]), res.x[1], interpolator((res.x[0], res.x[1])))
            )
