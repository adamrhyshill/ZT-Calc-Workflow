...
def _check_update_amset_dataset(
    df, check_uniform=True, convert_sigma_s_cm=True, calculate_pf_mw_m_k2=True
):
    ...
    * (re)calculate the power factors in mW m^-1 K^-2.
    ...
    if check_uniform:
        ...
        for n in n_vals:
            ...
            if len(t_check) != len(t_vals):
                raise Exception(
                    "Incomplete set of temperatures for n = " "{0:.3e}".format(n)
                )

            if not np.allclose(t_vals, sorted(t_check)):
                raise Exception(
                    "Inconsistent set of temperatures for n = " "{0:.3e}".format(n)
                )
    else:
        warnings.warn(
            "check_uniform is set to False - other functions may "
            "not work as expected on non-uniform data.",
            UserWarning,
        )
        # Code added to enforce uniformity - allows non-uniform data to be plotted without crashing
        # Clean up values with missing data
        # List all combinations of n and t
        combined = [n_vals, t_vals]
        df1 = pd.DataFrame(columns=["n", "t"], data=list(itertools.product(*combined)))
        # Combine into dataframe to show missing data
        df1 = df1.merge(df, how="left")
        # Find rows with any missing data (NaN in any column)
        df1 = df1[df1.isnull().any(axis=1)]
        # Find any n values with missing data
        n_drop = sorted(df1["n"].unique())
        # Drop n values without complete data
        df = df[df.n.isin(n_drop) == False]
        # Continue with usual read
        t_vals = sorted(df["t"].unique())
        n_vals = sorted(df["n"].unique())
    ...
    has_pf = (
        "pf_xx" in df.columns
        and "pf_yy" in df.columns
        and "pf_zz" in df.columns
        and "pf_ave" in df.columns
    )
    ...
    if calculate_pf_mw_m_k2 or not has_pf:
        key_sets = [
            ...
            ("s_yy", "sigma_yy", "pf_yy"),
            ("s_zz", "sigma_zz", "pf_zz"),
            ("s_ave", "sigma_ave", "pf_ave"),
        ]

        for k_s, k_sigma, k_pf in key_sets:
            df.loc[:, k_pf] = 1.0e3 * (
                (1.0e-6 * df[k_s].to_numpy()) ** 2 * df[k_sigma].to_numpy()
            )
    else:
        warnings.warn(
            "calculate_pf_mw_m_k2 is set to False - other functions may not "
            "work as expected if data is in different units.",
            UserWarning,
        )
...
_READ_AMSET_HEADER_MAP = {
    ...
    "Temperature": "t",
    "Conducitivty x": "sigma_xx",
    "Conducitivty y": "sigma_yy",
    "Conducitivty z": "sigma_zz",
    "Conducitivty ave": "sigma_ave",
    "Seebeck x": "s_xx",
    "Seebeck y": "s_yy",
    "Seebeck z": "s_zz",
    "Seebeck ave": "s_ave",
...
def read_amset_csv(file_path, **kwargs):
    ...
    df = read_validate_csv(
        file_path,
        header_map=_READ_AMSET_HEADER_MAP,
