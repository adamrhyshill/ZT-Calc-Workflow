...
def setup_matplotlib(sans_serif=True, font_size=8, linewidth=0.5):
    ...
    if sans_serif:
        mpl.rc(
            "font",
            **{"family": "sans-serif", "size": font_size, "sans-serif": "Calibri"}
        )
        ...
        mpl.rc("font", **{"family": "serif", "size": font_size, "serif": "Times"})
...
def cscale(n, h1, h2):
    ...
    return [hsv_to_rgb(((h / 360.0) % 1.0, 1.0, 1.0)) for h in np.linspace(h1, h2, n)]
