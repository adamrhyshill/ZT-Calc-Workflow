...
if __name__ == "__main__":
    ...
    input_files = {
        "SnS": (
            r"SnS-Pnma-AMSET-p.csv",
            r"SnS-Pnma-AMSET-n.csv",
            r"SnS-kappa-m16816.Prim.csv",
        ),
        "SnSe": (
            r"SnSe-Pnma-AMSET-p.csv",
            r"SnSe-Pnma-AMSET-n.csv",
    ...
    for k, (amset_p, amset_n, kappa) in input_files.items():
        ...
        zt_data[k] = (
            dataset_to_2d(zt_dataset_from_data(amset_data_p, kappa_data)),
            dataset_to_2d(zt_dataset_from_data(amset_data_n, kappa_data)),
        )
    ...
    for r in range(2):
        for c in range(2):
            subplot_axes.append(plt.subplot(grid_spec[4 * r + 1 : 4 * (r + 1) + 1, c]))
